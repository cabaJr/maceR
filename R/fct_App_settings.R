...
App_settings <- R6::R6Class("App_settings",
                            public = list(
                              ...
                              setLD = function(x, light, ddVal, ddStart){
                                ...
                                SLLD <- switch(ddVal,
                                               "Yes" = {stat_ld_annotations(height = 1, alpha = Llpha1, outline = NA, period = hours(24), l_duration = hours(light), x_limits = c(0, per1End), phase = 0, ld_colours = c(NA, "black"))},
                                               "No" = {stat_ld_annotations(height = 1, alpha = Llpha1, outline = NA, period = hours(24), l_duration = hours(light), phase = 0, ld_colours = c(NA, "black"))})
                                # check the rest of the function from here + implement value gathering inside server and function call
                                SLDD <- switch(ddVal,
                                               "Yes" = {stat_ld_annotations(height = 1, alpha = Llpha1, outline = NA, period = hours(24), l_duration = hours(0), x_limits = c(per1End, length1), phase = 0, ld_colours = c(NA, "black"))},
                                               "No" = {NULL})
                                DPLD <- stat_ld_annotations(height = 1, alpha = Llpha2, outline = NA, period = hours(24), l_duration = hours(light), phase = 0, ld_colours = c(NA, "black"))
                                DPDD1 <- switch(ddVal,
                                                "Yes" = {stat_ld_annotations(height = DDStart1, alpha = Llpha2, outline = NA, period = hours(24), l_duration = hours(DDlength), x_limits = c(hours(0), hours(24)), phase = hours(12), ld_colours = c(NA, "black"))},
                                                "No" = {NULL})
                                DPDD2 <- switch(ddVal,
                                                "Yes" = {stat_ld_annotations(height = DDStart2, alpha = Llpha2, outline = NA, period = hours(24), l_duration = hours(DDlength), x_limits = c(hours(24), hours(48)), phase = hours(12), ld_colours = c(NA, "black"))},
                                                "No" = {NULL})
