...
Custom_tables <- R6::R6Class("Custom_tables",
                             list(
                               ...
                               average_day = list(),
                               periodograms = list(),
                               cacheKeys = dplyr::tibble(
                                 "table" = seq(1:8), #table to store keys of hashed tables
                                 "key" = 0),
                               #' compile
                               #'
                               ...
                               #' CheckIf
                               #'
                               #' @param env env containing App_settings
                               #' @param subsetPlot Yes/No value
                               #'
                               checkIf = function(env, subsetPlot){#to add option to check for presence of different tables
                                 ...
                                 self$behavrTable(env = env, subsetVal = subsetPlot)
                                 # }else{}
                               },
                               #                            
                               #' behavrTable
                               ...
                               #'
                               behavrTable = function(env, subsetVal){
                                 d2 <- NULL
                                 browser()
                                 ...
                                 switch(subsetVal,
                                        "Yes" = {
                                          ...
                                          req(!is.null(idlist))
                                          toLoad <- env$subsetting$miceListFiltered$pos
                                          range = c((env$subsetting$timespan[1]*1440),(env$subsetting$timespan[2]*1440))
                                        },
                                        "No" = {
                               ...
                               #'     methods
                               #'     
                               computePer = function(method, periodRange, env, subsetVal) {
                                 # browser()
                                 # switch(subsetVal,
                                 #        "Yes" = {
                                 #          idlist = env$subsetting$miceListFiltered
                                 ...
                                 switch(perFun,
                                        "chi_sq_periodogram" = {
                                          period <- zeitgebr::periodogram(Activity, 
                                                                          data, 
                                                                          period_range = c(behavr::hours(vals[1]), 
                                                                                           behavr::hours(vals[2])),
                                        ...
                                        "fourier_periodogram" = {
                                          period <- zeitgebr::periodogram(Activity, 
                                                                          ...
                                                                          period_range = c(behavr::hours(vals[1]), 
                                                                                           behavr::hours(vals[2])),
                                                                          resample_rate = 1/behavr::mins(6), 
                                                                          alpha = 0.05, FUN = zeitgebr::fourier_periodogram)
                                        },
                                        "cwt_periodogram" = {
                                          period <- zeitgebr::periodogram(Activity, 
                                                                          data, 
